...
void tablo_proto_parser(uint8_t *proto_buf)//
{
   ...
   for(i=0;i<IND_ALL_NUM;i++)//обнулим флаги обновлени€ данных индикаторов
   {
...
//						  num_buf[chr_counter]='\0';
   ...
   for(i=0;i<IND_ALL_NUM;i++)//дл€ незадействованных индикаторов-гасим
   {
...
uint8_t str_to_ind(struct indicator *ind,uint8_t *str)
{
    ...
    str_len=strlen(str);//!!!
        ...
        if(str_len>10)//максимальна€ длина строки индикатора с точками
        {
        	str_len=10;
        }
        ...
        for(i=(str_len-1);i>=0;i--)//
        {
            ...
            {
               	uint8_t sym=Sym_table[1][(str[i]-0x30)];//коррекци€ ошибки индикатора
               	sym&=0x9F;
            ...
            for(j=10;j<SYM_TAB_LEN;j++)//
            {
               ...
               {
                  	uint8_t sym=Sym_table[1][j];//коррекци€ ошибки индикатора
                  	sym&=0x9F;
